        ...
        .then(() => {
            ...
            .then((users: UserModel[]) => {
                users.forEach((user, index) => {
                    ...
                    exec(`docker create -v ${process.env.NODE_ENV}_${user.company.name.split(" ").join("-")}_${req.body.projectName}:/home/botium-bindings/samples/botframework/spec/convo --name ${process.env.NODE_ENV}_${user.company.name.split(" ").join("-")}_${req.body.projectName}_${user.userName} -e PROJECTNAME='${req.body.projectName}' -e CONTAINERMODE='dialogflow' -e DIALOGFLOW_PROJECT_ID='${req.body.dialogflowProjectId}' -e DIALOGFLOW_CLIENT_EMAIL='${req.body.dialogflowClientEmail}' -e DIALOGFLOW_PRIVATE_KEY='${req.body.dialogflowPrivateKey}' -e COMPANY_ID='${req.body.companyId}' -e CHATBOT_ID='${prevChatbot.id}' -e USER_ID='${user.id}' -e HOST='${process.env.HOST}' tchikladze/chatbot:latest sh -c "${crontab} service cron restart ; cd /home/botium-bindings/samples/botframework/node_modules/jasmine ; npm run dotenvInit ; cd /home/botium-bindings/samples/botframework/node_modules/botium-cli ; npm run dotenvInit ; cd ../.. ; mkdir -p logs/${user.company.name.split(" ").join("-")}/${req.body.projectName}/${user.userName} ; npm run generateBotium && npm run emulator"`, (err, stdout, stderr) => {
                        if (err)
                            console.log(err);
                    });
                });

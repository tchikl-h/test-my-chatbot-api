    ...
    .then((chatbots: ChatbotModel[]) => {
        chatbots.forEach((chatbot: ChatbotModel) => {
            ...
            .then((users: UserModel[]) => {
                users.forEach(user => {
                    console.log(`docker ps -aqf "name=${process.env.NODE_ENV}_${user.company.name.split(" ").join("-")}_${chatbot.project_name}_${user.userName}"`);
                    exec(`docker ps -aqf "name=${process.env.NODE_ENV}_${user.company.name.split(" ").join("-")}_${chatbot.project_name}_${user.userName}"`, (err, stdout, stderr) => {
